...
class Model(object):
    ...
    def init_placeholders(self):
        ...
        # Review embedding placeholder (processed review embedding of size 32)
        self.review_embedding = tf.placeholder(tf.float32, [None, 32])
    ...
    def build_model(self, item_cate_list):
        ...
        # item embedding lookup and concatenation with category embedding
        ...
        # Incorporate the processed review embedding:
        # Project the review embedding (size 32) to the same dimension as u_t
        # and add it to the user representation.
        review_proj = tf.layers.dense(self.review_embedding, self.config['hidden_units'],
                                      activation=None, name='review_proj')
        u_t = tf.add(u_t, review_proj)

        # Final logit calculation: dot product between the updated user representation and item embedding
        self.logits = tf.reduce_sum(tf.multiply(u_t, i_emb), -1) + i_b

        # Eval: compute scores for all items
        self.eval_logits = tf.matmul(u_t, all_emb, transpose_b=True) + item_b
        self.i64 = tf.cast(self.i, tf.int64)
        with tf.variable_scope("metric"):
            # precision_at_k
            self.prec_1, self.prec_update_1 = tf.metrics.precision_at_k(labels=self.i64, predictions=self.eval_logits,
                                                                        k=1)
            self.prec_10, self.prec_update_10 = tf.metrics.precision_at_k(labels=self.i64, predictions=self.eval_logits,
                                                                          k=10)
            self.prec_20, self.prec_update_20 = tf.metrics.precision_at_k(labels=self.i64, predictions=self.eval_logits,
                                                                          k=20)
            self.prec_30, self.prec_update_30 = tf.metrics.precision_at_k(labels=self.i64, predictions=self.eval_logits,
                                                                          k=30)
            self.prec_40, self.prec_update_40 = tf.metrics.precision_at_k(labels=self.i64, predictions=self.eval_logits,
                                                                          k=40)
            self.prec_50, self.prec_update_50 = tf.metrics.precision_at_k(labels=self.i64, predictions=self.eval_logits,
                                                                          k=50)
            # recall_at_k
            self.recall_1, self.recall_update_1 = tf.metrics.recall_at_k(labels=self.i64, predictions=self.eval_logits,
                                                                         k=1)
            self.recall_10, self.recall_update_10 = tf.metrics.recall_at_k(labels=self.i64,
                                                                           predictions=self.eval_logits, k=10)
            self.recall_20, self.recall_update_20 = tf.metrics.recall_at_k(labels=self.i64,
                                                                           predictions=self.eval_logits, k=20)
            self.recall_30, self.recall_update_30 = tf.metrics.recall_at_k(labels=self.i64,
                                                                           predictions=self.eval_logits, k=30)
            self.recall_40, self.recall_update_40 = tf.metrics.recall_at_k(labels=self.i64,
                                                                           predictions=self.eval_logits, k=40)
            self.recall_50, self.recall_update_50 = tf.metrics.recall_at_k(labels=self.i64,
                                                                           predictions=self.eval_logits, k=50)
        ...
        # Loss: cross-entropy plus L2 regularization
        l2_norm = tf.add_n([
    ...
    def eval_auc(self, sess, batch):
        # Evaluate using positive and negative items
...
def linear(args, output_size, bias, bias_start=0.0, scope=None, squeeze=False, wd=0.0, keep_prob=1.0,
    ...
    if keep_prob < 1.0:
        ...
        flat_args = [tf.cond(is_training, lambda: tf.nn.dropout(arg, keep_prob), lambda: arg)
                     # for dense layer [(-1, d)]
